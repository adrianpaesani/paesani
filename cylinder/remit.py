...
class CylinderRemit(models.Model):
	...
	def action_done(self):
		...
		for remit in self:
			remit.remit_line_out._action_status_done(self.date_remit, self.partner_id.id, self.location_out_id.id)
			remit.remit_line_in._action_status_done(self.date_remit, self.partner_id.id, self.location_in_id.id)
...
class CylinderRemitLineOut(models.Model):
	...
	@api.multi
	def _action_status_done(self, date_remit, partner_id, location_id):
...
class CylinderRemitLineIn(models.Model):
	...
	cap = fields.Boolean()
	days_rented = fields.Integer('Days Rented')
	...
	def cylinder_id_change(self):
		...
		if cylinder.last_rental_date:
			self.days_rented = ((fields.Datetime.from_string(fields.Date.today()) - fields.Datetime.from_string(cylinder.last_rental_date))).days
		else:
			self.days_rented = 0
		self.cap = cylinder.cap
