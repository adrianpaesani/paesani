...
class SalesCommissionPayment(models.TransientModel):
    ...
    def generate_invoice(self):
        ...
        commission_account_id = self.env['ir.values'].sudo().get_default('account.config.settings',
                                                                         'commission_account_id')
        if not commission_account_id:
            raise Warning(_('Account not found. Please configure it first.'))
        else:
...
class WizardCommissionSummary(models.TransientModel):
    ...
    def print_commission_report(self):
        ...
        return self.env['report'].get_action(self, 'sales_commission.print_commission_summary_template', data=datas)
